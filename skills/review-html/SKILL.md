...
4. **轻量**：总 HTML 大小控制在 700 行以内，尽量精简。CSS 和 JS 均为少量内联。
...
4. 自动确保 `pi-review/` 文件夹存在于项目根目录，若不存在则创建；并将 `pi-review/` 加入 `.gitignore`（若未包含）。
5. HTML 文件命名格式：`年月日时分-任务简述(极简10词以内)-index.html`，若同名文件已存在则编号递增（如 `index1.html`）。
6. 直接输出 HTML 文件到 `pi-review/` 目录，**不要添加任何解释性文字**，仅简短说明工作完成和输出文件路径即可。
